...
def register_routes(app):
    ...
    # ---------------- REFRESH CI + RUN ML + AUTO REMEDIATION + GMAIL ALERTS ----------------
    @app.route("/refresh-ci", methods=["GET", "POST"])
    @login_required
    def refresh_ci():
        ...
        try:
            ...
            subprocess.run(
                [sys.executable, "ci_monitoring/email_alerts.py"],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            flash(
                "CI data, ML prediction, auto remediation, and Gmail alerts completed successfully!",
                "success",
            )
